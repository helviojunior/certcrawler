# CertCrawler instalation procedures 
...
url=$(curl -s https://api.github.com/repos/helviojunior/certcrawler/releases | jq -r '[ .[] | {id: .id, tag_name: .tag_name, assets: [ .assets[] | select(.name|match("linux-amd64.tar.gz$")) | {name: .name, browser_download_url: .browser_download_url} ]} | select(.assets != []) ] | sort_by(.id) | reverse | first(.[].assets[]) | .browser_download_url')

cd /tmp
rm -rf certcrawler-latest.tar.gz certcrawler
wget -nv -O certcrawler-latest.tar.gz "$url"
tar -xzf certcrawler-latest.tar.gz

rsync -av certcrawler /usr/local/sbin/
chmod +x /usr/local/sbin/certcrawler
...
url=$(curl -s https://api.github.com/repos/helviojunior/certcrawler/releases | jq -r --arg filename "darwin-${arch}.tar.gz\$" '[ .[] | {id: .id, tag_name: .tag_name, assets: [ .assets[] | select(.name|match($filename)) | {name: .name, browser_download_url: .browser_download_url} ]} | select(.assets != []) ] | sort_by(.id) | reverse | first(.[].assets[]) | .browser_download_url')

cd /tmp
rm -rf certcrawler-latest.tar.gz certcrawler
curl -sS -L -o certcrawler-latest.tar.gz "$url"
tar -xzf certcrawler-latest.tar.gz
...
```
# Download latest helviojunior/certcrawler release from github
function Invoke-Downloadcertcrawler {

    $repo = "helviojunior/certcrawler"
    ...
    $file = "certcrawler-latest$extension"
    ...
    Write-Host "Getting certcrawler version banner"
    . $name version 
}
...
Function Get-AssetData {
    ...
    if (Get-Member -inputobject $Release -name "assets" -Membertype Properties) {
        ...
        {
            If ($asset.name.Contains("certcrawler-") -and $asset.name.Contains("$platform-$arch$extension")) { Return $asset }
        }
...
```
git clone https://github.com/helviojunior/certcrawler.git
cd certcrawler
go get ./...
go build
```
...
```
go install .
ln -s /root/go/bin/certcrawler /usr/bin/certcrawler
```
